...
class UserService {
  ...
  async updateUserMe(req, id, body) {
    const userUpdate = await UserModel.findOneAndUpdate(id, body, {
      runValidators: true,
      new: true
    })
    if (!userUpdate) {
  ...
  async forgotPassword(req, user) {
    ...
    if (user) {
      ...
      await UserModel.findByIdAndUpdate({
        ...
        resetPasswordDataExpires: Date.now() + 86400000
      }, {
        new: true
      })
  ...
  async resetPassword(req, token, body) {
    ...
    if (user && body.newPassword === body.verifyPassword) {
      ...
      const newBody = {
        ...
        resetPasswordDataExpires: undefined
      }
  ...
  async updatePassword(id, body) {
    ...
    const verifyOldPassword = await UserModel.findUserByCredentials(user.email, req.body.oldPassword)
    if (verifyOldPassword) {
      ...
      if (user && body.newPassword === body.verifyPassword) {
        ...
        const updateUser = await UserModel.findOneAndUpdate(id, { ...newBody }, {
          runValidators: true,
          new: true
        })
        return updateUser
